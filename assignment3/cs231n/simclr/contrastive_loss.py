...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sims_1 = torch.tensor([sim(z_k, z) for z in out[np.arange(2*N)!=k]])
        
        l1 = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / torch.sum(torch.exp(sims_1/tau)))

        sims_2 = torch.tensor([sim(z_k_N, z) for z in out[np.arange(2*N)!=k+N]])

        l2 = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / torch.sum(torch.exp(sims_2/tau)))
...
def sim_positive_pairs(out_left, out_right):
    ...
    prod_internos = torch.sum(out_left * out_right, dim=1, keepdim=True)

    norma_izq = torch.linalg.norm(out_left, dim=1, keepdim=True)
    norma_der = torch.linalg.norm(out_right, dim=1, keepdim=True)

    pos_pairs = prod_internos / (norma_izq * norma_der)
...
def compute_sim_matrix(out):
    ...
    prod_internos = torch.matmul(out, out.T)
    normas = torch.linalg.norm(out, dim=1, keepdims=True)

    sim_matrix = prod_internos / torch.matmul(normas, normas.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    loss = -torch.mean(torch.log(torch.concat((numerator, numerator), dim=0) / denom))
