...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions_embed = self.positional_encoding(self.embedding(captions))

        features_proj = self.visual_projection(features).unsqueeze(1)

        tgt_mask = torch.tril(torch.ones((T,T)))

        t_output = self.transformer(captions_embed, features_proj, tgt_mask=tgt_mask)

        scores = self.output(t_output) 
