...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pos_i, pos_j = torch.meshgrid(torch.arange(max_len), torch.arange(embed_dim))
        ang_cos = pos_i * 10000**(-pos_j/embed_dim)
        ang_sin = pos_i * 10000**(-(pos_j-1)/embed_dim)
        pe[0, :, 0::2] = torch.sin(ang_cos[:, 0::2])
        pe[0, :, 1::2] = torch.cos(ang_sin[:, 1::2])
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        output = output.transpose(1, 2).reshape(N, S, self.emd_dim)
