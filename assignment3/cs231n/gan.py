...
def discriminator_loss(logits_real, logits_fake):
    ...
    target_real = torch.ones(N, device=logits_real.device)
    loss_real = bce_loss(logits_real, target_real)

    target_fake = torch.zeros(N, device=logits_fake.device)
...
def generator_loss(logits_fake):
    ...
    target = torch.ones(N, device=logits_fake.device)
