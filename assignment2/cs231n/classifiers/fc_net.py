    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          if self.normalization == None:
            z, cache[i] = affine_relu_forward(z, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
          if self.normalization == "batchnorm":
            z, cache[i] = affine_batchnorm_relu_forward(z, self.params[f'W{i+1}'], 
            self.params[f'b{i+1}'], self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], self.bn_params[i])
          if self.use_dropout:
            z, cache_do[i] = dropout_forward(z, self.dropout_param)
        
        z, cache[self.num_layers-1] = affine_forward(z, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        ...
        for i in reversed(range(self.num_layers-1)):
          if self.use_dropout:
            dout = dropout_backward(dout, cache_do[i])
          if self.normalization == None: 
