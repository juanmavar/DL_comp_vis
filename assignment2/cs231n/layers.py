...
def softmax_loss(x, y):
    ...
    loss = -np.mean(np.log(probs_loss))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = np.zeros(shape=(N, F, H_p, W_p))
    for n in range(N):
      for f in range(F):
        for h_p in range(H_p):
          for w_p in range(W_p):
            out[n,f, h_p, w_p] = np.sum(x_pad[n,:,h_p*stride:(h_p*stride+HH) , w_p*stride:(w_p*stride+WW)]* w[f,:,:,:]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
      for f in range(F):
        for h_p in range(H_p):
          for w_p in range(W_p):
            dw[f,:,:,:] += x_pad[n,:,h_p*stride:(h_p*stride+HH) , w_p*stride:(w_p*stride+WW)] * dout[n,f,h_p,w_p]

            dx_pad[n,:,h_p*stride:(h_p*stride+HH) , w_p*stride:(w_p*stride+WW)] += w[f,:,:,:] * dout[n,f,h_p,w_p]
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_p = int((H - p_H) / stride) + 1
    W_p = int((W - p_W) / stride) + 1

    out = np.zeros(shape=(N, C, H_p, W_p))

    for n in range(N):
      for c in range(C):
        for h_p in range(0, H_p):
          for w_p in range(0, W_p):
            out[n,c,h_p,w_p] = np.max(x[n,c,h_p*stride:(h_p*stride+p_H) , w_p*stride:(w_p*stride+p_W)])
    ...
    for n in range(N):
      for c in range(C):
        for h_p in range(0, H_p):
          for w_p in range(0, W_p):
            # Region de pooling
            x_pr = x[n,c,h_p*stride:(h_p*stride+p_H) , w_p*stride:(w_p*stride+p_W)]
            # Indices del maximo
            max_i = np.unravel_index(np.argmax(x_pr), x_pr.shape)
            # Se propaga el gradiente del maximo, los demas siguen siendo 0
            dx[n, c, h_p*stride + max_i[0], w_p*stride + max_i[1]] += dout[n, c, h_p, w_p]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_f, cache = batchnorm_forward(x_f, gamma, beta, bn_param)
    
    out = out_f.reshape(N, H, W, C).transpose(0, 3, 1, 2)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_f = dout_t.reshape(-1, C)

    dx_f, dgamma, dbeta = batchnorm_backward(dout_f, cache)
